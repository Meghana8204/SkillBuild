...
const getProgrammeSummary = asyncHandler(async (req, res) => {
  const [institutionUsers, batches, students, trainers, sessions, attendance] = await Promise.all([
    User.find({ role: "institution" }).select("_id name email").lean(),
    Batch.find({}).lean(),
  ...
  const institutionSummaries = await Promise.all(
    institutionUsers.map(async (institution) => {
      const institutionBatches = batches.filter((batch) =>
        isSameId(batch.institutionId, institution._id)
      );
      const batchSummaries = await Promise.all(institutionBatches.map(buildBatchSummary));
      const institutionPossibleMarks = batchSummaries.reduce(
        (sum, batchSummary) => sum + batchSummary.totalSessions * batchSummary.totalStudents,
        0
      );
      const institutionAttendedMarks = batchSummaries.reduce(
        (sum, batchSummary) =>
          sum +
          batchSummary.students.reduce(
            (studentSum, student) => studentSum + student.present + student.late,
            0
          ),
        0
      ...
      batchSummaries.forEach((batchSummary) => {
        batchSummary.students.forEach((student) => {
          institutionStudentIds.add(student.studentId.toString());
        });
      });
      ...
      return {
        ...
        totalBatches: institutionBatches.length,
        totalStudents: institutionStudentIds.size,
        totalSessions: batchSummaries.reduce((sum, batchSummary) => sum + batchSummary.totalSessions, 0),
        attendancePercentage: percent(institutionAttendedMarks, institutionPossibleMarks),
        batches: batchSummaries
      };
    })
  );
  ...
  res.status(200).json({
    ...
    data: {
      summary: {
        totalInstitutions: institutionUsers.length,
        totalBatches: batches.length,
